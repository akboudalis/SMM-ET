...
fclose(fid); fid = fopen(fname); line = line + 1;
freq_vs_T = textscan(fid, '%f',1, 'Delimiter', ',', 'Headerlines', line,'CommentStyle','%'); freq_vs_T = cell2mat(freq_vs_T); freq_vs_T = freq_vs_T';
fclose(fid); fid = fopen(fname); line = line + 1;
...
fclose(fid); fid = fopen(fname); line = line + 1;
T_vs_freq = textscan(fid, '%f',1, 'Delimiter', ',', 'Headerlines', line,'CommentStyle','%'); T_vs_freq = cell2mat(T_vs_freq); T_vs_freq = T_vs_freq';
fclose(fid); fid = fopen(fname); line = line + 1;
% ---
...
% Define the domains of the experiments
% Vs T experiments
T_vs_T = linspace(T_vs_T(1),T_vs_T(2),T_vs_T(3)); % In K
% freq_vs_T = [1e2]; % In Hz
% freq_vs_T = logspace(-1,4,20); % In Hz
...
% T_vs_H = [2];
% freq_vs_H = [0.1, 1, 10, 1e2, 1e3, 1e4]; % In Hz
% w_vs_H = 2*pi*freq_vs_H;
...
% ---------Are there experimental dc data available?------------
% chi vs T
if exist('chi_vs_T_dc.dat')
    [Texp_dc,chi_dc] = textread('chi_vs_T_dc.dat','%f %f');
    chi_vs_T = spline(Texp_dc,chi_dc,T_vs_T);
...
% The field-sweep domain for the hysteresis experiments
...
H_QTM_vs_H = zeros(1,length(H_vs_H));
for k = 1:length(H_QTM)
 % Find the indexes of the sweep field closest to the level crossings
[dH, index(k)] = min( abs( H_vs_H - H_QTM(k) ) );
end
for l = min(index):max(index)
% Assign a non-zero value to elements of H_sweep_QTM where level-crossings occur
H_QTM_vs_H(index) = 1;%H_sweep_up(sweep_index); %
end
dH_QTM_vs_H = bwdist(H_QTM_vs_H); % Calculate distances for each point of the field sweep from the non-zero elementsfrom each non-zero element of H_sweep_QTM
dH_QTM_vs_H = dH_QTM_vs_H*max(H_QTM_vs_H)/max(dH_QTM_vs_H); % Scale the distances to the H_sweep_up vector values (the actual field sweep)

% Distances from level crossings for vs T experiments
H_QTM_vs_T = zeros(1,length(H_vs_T));
for k = 1:length(H_QTM)
 % Find the indexes of the current field closest to the level crossings
[dH, index(k)] = min( abs( H_vs_T - H_QTM(k) ) );
end
for l = min(index):max(index)
% Assign a non-zero value to elements of H_sweep_QTM where level-crossings occur
H_QTM_vs_T(index) = 1;%H_sweep_up(sweep_index); %
end
if length(H_vs_T) >=3
dH_QTM_vs_T = bwdist(H_QTM_vs_T); % Calculate distances for each point of the field sweep from the non-zero elementsfrom each non-zero element of H_sweep_QTM
dH_QTM_vs_T = dH_QTM_vs_T*max(H_QTM_vs_T)/max(dH_QTM_vs_T); % Scale the distances to the H_sweep_up vector values (the actual field sweep)
else
    for l = 1:length(H_vs_T)
    dH_QTM_vs_T(l) = min(abs(H_vs_T(l) - H_QTM));
    end
end

% Distances from level crossings for vs freq experiments
H_QTM_vs_freq = zeros(1,length(H_vs_freq));
for k = 1:length(H_QTM)
 % Find the indexes of the sweep field closest to the level crossings
[dH, index(k)] = min( abs( H_vs_freq - H_QTM(k) ) );
end
for l = min(index):max(index)
% Assign a non-zero value to elements of H_sweep_QTM where level-crossings occur
H_QTM_vs_freq(index) = 1;%H_sweep_up(sweep_index); %
end
if length(H_vs_freq) >=3
dH_QTM_vs_freq = bwdist(H_QTM_vs_freq); % Calculate distances for each point of the field sweep from the non-zero elementsfrom each non-zero element of H_sweep_QTM
dH_QTM_vs_freq = dH_QTM_vs_freq*max(H_QTM_vs_freq)/max(dH_QTM_vs_freq); % Scale the distances to the H_sweep_up vector values (the actual field sweep)
else
    for l = 1:length(H_vs_freq)
    dH_QTM_vs_freq(l) = min(abs(H_vs_freq(l) - H_QTM));
    end
end

% Define functions 1/tau and tau = f(T,H). The parameters are the ones given above.

if mod(Sys.S,1) ~= 0 % Kramers ion
...
func_tau_vs_TH = @(Ueff,b_Orb,n_Raman,b_Raman,b_Direct,b_QTM1,b_QTM2,x,y,z)... % x = T, y = H
    ...
    b_Direct * y.^4 .* x+...
    b_QTM1./(1 + b_QTM2 * z.^2));
else % non-Kramers ion
funct_tau_inv_vs_TH = @(Ueff,b_Orb,n_Raman,b_Raman,b_Direct,b_QTM1,b_QTM2,x,y)... % x = T, y = H
    b_Orb * exp(-Ueff./x)+...
    b_Raman * x.^n_Raman+...
    b_Direct * y.^2 .* x+...
    b_QTM1./(1 + b_QTM2 * z.^2); 
func_tau_vs_TH = @(Ueff,b_Orb,n_Raman,b_Raman,b_Direct,b_QTM1,b_QTM2,x,y)... % x = T, y = H
    1./(b_Orb * exp(-Ueff./x)+...
    b_Raman * x.^n_Raman+...
    b_Direct * y.^2 .* x+...
    b_QTM1./(1 + b_QTM2 * z.^2));
...
tau_inv_Direct_T = b_Direct * H_vs_T(i).^4 .* T_vs_T; tau_Direct = 1./tau_inv_Direct_T;
...
tau_inv_Direct_H = b_Direct * H_vs_H.^4 * T_vs_H(i); tau_Direct_H = 1./tau_inv_Direct_H;
...
% Assign a non-zero value to elements of H_sweep_QTM where level-crossings occur
H_sweep_QTM(sweep_index) = 1;%H_sweep_up(sweep_index); %
end
dH_QTM = bwdist(H_sweep_QTM); % Calculate distances for each point of the field sweep from the non-zero elementsfrom each non-zero element of H_sweep_QTM
dH_QTM = dH_QTM*max(H_sweep_QTM)/max(dH_QTM); % Scale the distances to the H_sweep_up vector values (the actual field sweep)
...
for i = 1:length(H_sweep_up)
    ...
    M_vs_H_up(i) = Meq_up(i) + (M_vs_H_up(i) - Meq_up(i)) * exp(-delay/tau_sweep(i)) ; % Magnetization after the delay of the measurement (if any)
end
